...
class StoreVariousDataTypesTransform extends JavaSparkSqlTransform {
  ...
  override def execute(sqlContext: JavaSparkSqlTransformContext): DataFrame = {

    sqlContext.getJavaSparkTransformContext.getJavaSparkContext.setLogLevel("DEBUG")
    //TODO remove the inputs
    //TODO
    val context:SQLContext = sqlContext.getSqlContext()
    val inputCollectionId = sqlContext.getJavaSparkTransformContext().getInputCollectionIds().get(0)
    val inputDataframe:DataFrame = context.table(inputCollectionId)
      .select("day", "entityName", "entityId")

    val variousTypes:JavaRDD[Row] = inputDataframe.map{ data =>

      val formatter = new SimpleDateFormat("yyyy-MM-dd")
      val date = formatter.parse(data.get(0).asInstanceOf[String])
...
//      val calendarInterval: CalendarInterval = new CalendarInterval(1, 123000L)
      ...
      Row(
        data.get(1),//String
        binaryType,//Binary
        data.get(2).isInstanceOf[Double],//Boolean
...
//      StructField("AnyDataType", AnyDataType, true)
    ...
    val outputModelDataframe = KoverseSparkSql.createDataFrame(variousTypes, context, outputSchema)
